...
export default function HabitList({ allHabitData = [], userId }) {
    ...
    return (
        <div>
            ...
            {/* ── 통계 카드 row ── */}
            {(() => {
                const dateSet = new Set(allHabitData.filter(d => d.count > 0).map(d => d.date))
                const now = new Date()
                const kstOffset = 9 * 60 * 60 * 1000
                const todayKST = new Date(Math.floor((now.getTime() + kstOffset) / 86400000) * 86400000 - kstOffset)
                let streak = 0
                let d = new Date(todayKST)
                while (true) {
                ...
                return (
                    <div className="responsive-stats-grid desktop-only" style={{ display: 'grid', gap: '16px', marginBottom: '32px' }}>
                        {[
                            ...
                            { label: '연속 출석', value: streak, sub: '일째', color: '#F59E0B' },
                        ].map(card => (
